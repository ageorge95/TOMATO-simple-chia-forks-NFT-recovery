...
**Parts of TOMATO's code have been taken from https://github.com/Flora-Network/flora-dev-cli**
...
- The script's' functionalities:
    - it can recover NFT pool rewards for chia and chia forks blockchains
    - it supports both v1 and v2 full node databases
    - it will display the coins that are unqualified to be recovered (yet) as well
    - it needs a running full node
    - it needs a synced full node db
...
If you want fast support (faster than with github issues that is), join our Discord server: https://discord.gg/7SQfypERzc

After the setup is done you can use the CLI interface to start the recovery.

![alt text](ReadMe_res/CLI_overview.JPG?raw=true)

# How to get the delayed puzzlehash
